...
func _process(delta: float) -> void:
	...
	visibility_smooth.advance_to(1.0 if camera.is_position_in_frustum(effective_sun_dir) else 0.0, delta)
	...
	if visible:
		...
		material_override.set_shader_parameter("sun_in_bounds", camera.is_position_in_frustum(effective_sun_dir))

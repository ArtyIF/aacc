...
@onready var _smoothed_position: Vector3 = global_position
...
func _process(delta: float) -> void:
	...
	_smoothed_direction = _smoothed_direction.slerp(_direction_target, delta * 5.0)
	_smoothed_position = _smoothed_position.lerp(_follow_node.global_position + (_smoothed_direction * 4.0) + (Vector3.UP * 2.0), delta * 60.0)

...
func _process(delta: float) -> void:
	...
	if up_vector_target.distance_to(smoothed_up_vector) > 0.001:
		smoothed_up_vector = smoothed_up_vector.slerp(up_vector_target, delta * 10.0)

	if direction_target.distance_to(smoothed_direction) > 0.001:
		smoothed_direction = smoothed_direction.slerp(direction_target, delta * smooth_amount)
